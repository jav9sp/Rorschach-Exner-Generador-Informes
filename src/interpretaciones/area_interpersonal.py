...
def interpretar_interpersonal(variables, estados_simples):
    ...
    # Paso 1: CDI y HVI
    cdi_txt = variables.get("CDI TXT", "")
    interpretaciones.append(cdi_txt)
    hvi_txt = variables.get("HVI TXT", "")
    interpretaciones.append(hvi_txt)

    # Paso 2: Relación a:p
    # TODO: Incorporar matices con P, T, Fd y Rigidez
    ...
    sum_t_txt = ""
    if sum_t == 0:
        sum_t_txt = f"Su capacidad para identificar sus necesidades de contacto es menor a lo esperado, lo que indica que {persona} tiende a asumir una postura más reservada y cautelosa en la interacción con otros."
    if sum_t == 1:
        sum_t_txt = f"Su capacidad para identificar sus necesidades de contacto es adecuada, por lo que {persona} cuenta con disposición a establecer contacto con los demás según lo esperado."
    if sum_t > 1:
        sum_t_txt = ""
    ...
    if ghr + phr >= 3:
        if phr >= ghr:
            ghr_phr_txt = f"Respecto a su eficacia interpersonal, se observan dificultades de adaptabilidad que llevan a {persona} a ser percibida de manera poco favorable por los demás. Además, dado que sus constructos sobre los demás contienen sesgos, su percepción de los otros y sobre los vínculos se encuentra alterada."

        if ghr > phr:
            ghr_phr_txt = f"Respecto a su eficacia interpersonal, se observa que {persona} es capaz de establecer vínculos positivos, profundos y empáticos con los demás."
