
def interpretar_interpersonal(variables, estados_simples):
    """
    Interpreta el área de relaciones interpersonales: contacto, representación de otros, vínculo, cooperación.
    """

    interpretaciones = []
    persona = "el evaluado" if variables["Genero"] == "M" else "la evaluada"

    # Paso 1: CDI y HVI
    cdi_txt = variables.get("CDI TXT", "Indefinido")
    interpretaciones.append(cdi_txt)
    hvi_txt = variables.get("HVI TXT", "Indefinido")
    interpretaciones.append(hvi_txt)

    # Paso 2: Relación a:p
    # TODO: Incorporar matices con P, T, Fd y Rigidez
    tot_a = variables.get("a", 0)
    tot_p = variables.get("p", 0)

    ap_txt = ""
    if tot_p > tot_a + 1:
        ap_txt = "Muestra una tendencia a asumir un rol pasivo en la interacción con los demás, por lo que deja que ellos tomen la iniciativa, evita asumir la responsabilidad de sus decisiones y espera que su entorno actúe en función de sus necesidades."

    interpretaciones.append(ap_txt)

    # Paso 3: Respuestas de comida (Fd) y textura (T)
    tot_t = variables.get("SumT", 0)
    tot_t_txt = ""
    if tot_t == 0:
        tot_t_txt = f"Su capacidad para identificar sus necesidades de contacto es menor a lo esperado, lo que indica que {persona} tiende a asumir una postura más reservada y cautelosa en la interacción con otros."
    if tot_t == 1:
        tot_t_txt = ""
    if tot_t > 1:
        tot_t_txt = ""

    # Paso 4: Análisis de los contenidos humanos
    todo_h = estados_simples.get("TodoH", "Indefinido")
    todo_h_txt = ""
    if todo_h == "muy alto":
        todo_h_txt = ""
    if todo_h == "alto":
        todo_h_txt = ""
    if todo_h == "normal":
        todo_h_txt = ""
    if todo_h == "bajo":
        todo_h_txt = "Su interés en el componente humano se encuentra por debajo de lo esperado"
    if todo_h == "muy bajo":
        todo_h_txt = ""

    # ? Añadir matices sobre el tipo de contenido H predominante

    parrafo_cont_h = f"{todo_h_txt}, En cuanto a cómo construye las conceptualizaciones de los demás, "

    interpretaciones.append(parrafo_cont_h)

    # Paso 5: GHR:PHR
    ghr = variables.get("GHR", 0)
    phr = variables.get("PHR", 0)

    ghr_phr_txt = ""
    if ghr + phr >= 3:
        if phr >= ghr:
            ghr_phr_txt = f"Respecto a su eficacia interpersonal, se observan dificultades de adaptabilidad que llevan a {persona} a ser percibida de manera poco favorable por los demás. Además, dado que sus constructos sobre los demás contienen sesgos, su percepción de los otros y sobre los vínculos se encuentra alterada."

        if ghr > phr:
            ghr_phr_txt = f"Respecto a su eficacia interpersonal, se observa que {persona} es capaz de establecer vínculos positivos, profundos y empáticos con los demás."

    # Paso 6: PER, COP y AG
    per = variables.get("PER", 0)  # * Significativo si es > 2
    cop = variables.get("COP", 0)
    ag = variables.get("AG", 0)

    cop_ag_txt = ""
    if cop == 0 and ag <= 1:
        cop_ag_txt = "Además, "

    parrafo_eficacia = f"{ghr_phr_txt}"

    interpretaciones.append(parrafo_eficacia)
    # Paso 7: Índice de aislamiento
    aisl = variables.get("Aisl/R", 0)

    if aisl > 0.33:
        pass
    if aisl > 0.25:
        pass

    # Paso 8: Contenidos de M y FM con pares.

    return interpretaciones
